    ...
    try {
      ...
      try {
        ...
        if (
          ...
          syncError.message.includes("no such table")
        ) {
          console.log(
            "⚠️ Database conflicts detected, force recreating to fix..."
          );
          try {
            await sequelize.sync({ force: true });
            console.log(
              "✅ Database forcefully recreated and models synchronized successfully."
            );
          } catch (forceError) {
            console.log("❌ Force recreation failed:", forceError.message);
            console.log(
              "⚠️ Continuing anyway - some functionality may not work."
            );
          }
        } else {
      ...
      try {
        ...
        console.log(
          "⚠️ Could not create server status record:",
          statusError.message
        );
